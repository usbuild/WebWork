<?php if (!Yii::app()->user->isGuest): ?>
    ...
    <div class="m-menu">
        ...
        <div class="menum">
            <ul>
                <li style="border-bottom: solid 1px #CCC;"><input type="text" id="add_new_tag" class="tag-text-input"
                                                                  title="添加标签"/></li>
                <?php foreach (Yii::app()->user->model->follow_tags as $tag): ?>

                    <li class="tag-item"><a href="<?= $this->createUrl('site/tagposts', array('tag' => $tag->tag)) ?>"
                                            class="mi"><span
                                class="icn icn-4"></span><span
                                class="txt"><?=$tag->tag?></span><span class="tag-close with-tip" data-tip="删除这个标签">X</span></a>
                    </li>
                <?php endforeach;?>
            </ul>
        </div>
...
cursor: auto;
color: #3D76BE;
margin-top: 40px;" id="cowriter-menu">协作博客
                </li>
                <?php foreach (Yii::app()->user->model->writers() as $b): ?>
                    <li class="tag-item" data-id="<?= $b->id ?>">
                        <a href="<?= $this->createUrl('site/writerposts/' . $b->id) ?>" class="mi">
                            ...
                            <span class="writer-close with-tip" data-tip="退出博客协作">X</span>
                            <span class="writer-add with-tip" data-tip="向博客发表文章">+</span>
                        </a>
                    </li>
                <?php endforeach?>
            </ul>
